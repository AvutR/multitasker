...
- USE THE TEAM'S INSTALLED CLAUDE CODE SKILLS BY DEFAULT. Prefer a skill over doing something by hand: reach for /build for feature work, /council to pressure-test a spec, /security-review before landing, and the principal-* agents where useful.
- Build DEEP, not shallow — cover the long tail of a workflow, not just the happy path.
- Keep Linear and Notion current as you work. When you start, finish, or block on work, update the relevant Linear issue status, post a progress comment, and update the Notion spec. ALWAYS do this through the Multitasker integration tools (post_standup, send_slack_message, update_linear_status, comment_on_linear_issue, update_linear_issue, post_weekly_project_update, update_notion_spec, update_notion_page) — they route through the approval policy and are audited. Do NOT call raw connector tools directly when a Multitasker tool exists.
